    ...
    def execute(self):
        if self.args.early_check:
            ...
            for game in range(1, len(actions_list) + 1):
                ...
                # idx = it.multi_index
                # if gold_N[idx] != self.agent.N[idx]:
                #     print(f"At index {idx}, gold N has {np.longdouble(gold_N[idx])}, N table has {self.agent.N[idx]}")
                # print(f"With a difference of {gold_N[idx]-self.agent.N[idx]} ")
                # it.iternext()
